...
-- '한국'에서 근무하는 사원들의 사번, 이름, 부서명, 직급명, 근무국가명 조회
...
SELECT * FROM LOCATION;	  -- NATIONAL_CODE									LOCATION_ID
...
--           딱 한번 뷰로 만들어 주면 그때부터는 뷰를 사용해서 간단하게 조회할 수 있음

-- 해당 계정이 가지고 있는 VIEW에 대한 정보를 조회할 수 있는 데이터 딕셔너리
SELECT * FROM USER_VIEWS;
-- 뷰는 논리적인 가상테이블 => 실직적으로 데이터를 저장하고 있지 않음
--						 (쿼리문을 TEXT 형태로 저장해놓음)
---------------------------------------------------------------------------------------------------------------
/*
 ...
 *     AS 서브쿼리;
 * 
 * 뷰 생성 시 기존에 중복된 이름의 뷰가 존재한다면 갱신(변경)해주고 없다면 새로운 뷰를 생성해줌
 */
