 ...
 * ACID
 * 
 * 1. Atomicity(원자성)   : 트랜잭션 내의 모든 작업은 전부 수행되거나, 전혀 수행되지 않아야 한다. 원칙
 * 2. Consistency(일관성) : 트랜잭션이 성공적으로 완료 된 후에도 데이터베이스는 유효한 상태를 유지해야한다는 원칙
 * 3. Isolation(고립성)   : 동시에 실행되는 여러 트랜잭션이 상호간에 영향을 끼치지 않도록 하는 원칙
 * 4. Durablity(지속성)   : 트랜잭션이 성공적으로 수행되었다면, 시스템에 문제가 발생하더라도 영구적으로 반영이 되어야 한다.
 * 
 * 
...
-- 2개 행이 삭제된 시점에 SAVEPOINT 가정
SAVEPOINT DELETE2ROWS;

SELETE * FROM EMP_COPY;
