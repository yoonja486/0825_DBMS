...
SELECT
       SUM(SALARY)
  FROM
       EMPLOYEE;	--> EMPLOYEE테이블에 존재하는 모든 사원들을 하나의 그룹으로 묶어서 합계를 구한 결과
...
/*
 * < 실행 순서 >
 * 5번 : SELECT		조회하고자하는 컬럼명 / 함수식 / 산술연산식 / 리터럴값 / * AS "별칭"
 * 1번 :   FROM		조회하고자하는 테이블명
 * 2번 :  WHERE		조건식
 * 3번 :  GROUP BY   그룹 기준에 해당하는 컬럼명 / 함수식
 * 4번 : HAVING		그룹함수로 만들어줄 조건식
 * 6번 :  ORDER BY	정렬기준으로 사용할 컬럼명 / 함수식 / 별칭 / 컬럼 순번
 */

-- EMPLOYEE 테이블로부터 각 직급별(JOB_CODE)별 총 급여합이 1000만원 이상인 직급의 직급코드, 급여합을 조회
-- 단, 직급코드가 J6인 그룹은 제외
SELECT
       SUM(SALARY)
     , JOB_CODE  
